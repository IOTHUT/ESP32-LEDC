...
#define LEDC_BASE_FREQ     10       // 5 kHz de clock base para o LED
#define LED_PIN            2        // led pin

int ledBrightness = 0;    // variavel para definir o brilho do led
int stepIncrement = 5;    // degrau de incremento ou decremento para o brilho do led

void ledcAnalogWrite(uint8_t channel, uint32_t value, uint32_t valueMax = 255) {
  // calcula o duty cycle (deve estar )
  uint32_t duty = (8191 / valueMax) * min(value, valueMax);
  ledcWrite(channel, duty);  // aplica de fato as mudanças ao pino associado ao canal
}
...
void loop() {
  // "escreve" mudanças no canal do led (aplica o brilho selecionado)
